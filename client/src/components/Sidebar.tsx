...
SidebarProps) => {
    ...
    try {
        const confirm = window.confirm('Are you sure you want to switch to this version? Any unsaved changes will be lost.')
        if(!confirm) return;
        ...
        setIsGenerating(false);
        toast.error(error?.response?.data?.message || error.message || 'Failed to rollback version');
        console.log(error);
    }
    }
    const handleRevisions = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setIsGenerating(false);
            toast.error(error?.response?.data?.message || error.message || 'Failed to submit revision');
            console.log(error);
            clearInterval(interval)
        }
    }
    ...
    <div className={`h-full sm:max-w-sm rounded-xl bg-gray-900 border-gray-800 transition-all ${isMenuOpen ? "max-sm:w-0 overflow-hidden" : 'w-full'} `}>
        <div className='flex flex-col h-full'>
            ...
            <div className='flex-1 overflow-y-auto no-scrollbar px-3 flex flex-col gap-4'>
                ...
                .sort((a,b)=> new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()).map((message) => { const isMessage = 'content' in message;
                            ...
                            <div key={ver.id} className='w-4/5 mx-auto my-2 p-3 rounded-xl bg-gray-800 text-gray-100 shadow flex flex-col gap-2'>
                                ...
                                <div className='flex items-center justify-between'>
                                    {project.current_version_index === ver.id ? (
                                        ...
                                        <button onClick={() => handleRollback(ver.id)} className='px-3 py-1 rounded-md text-xs bg-indigo-500 hover:bg-indigo-600 text-white'>Switch to this version</button>
                    ...
                    <div className='flex items-start gap-3 justify-start'>
                        <div className = 'w-8 h-8 rounded-full bg-linear-to-br from-indigo-600 to-indigo-700 flex items-center justify-center'>
                            ...
                            <div className='flex gap-1.5 h-full items-end'>
                                <span className = 'size-2 rounded-full animated-bounce bg-gray-600' style={{animationDelay : '0s'}} />
                                <span className = 'size-2 rounded-full animated-bounce bg-gray-600' style={{animationDelay : '0.2s'}} />
                                <span className = 'size-2 rounded-full animated-bounce bg-gray-600' style={{animationDelay : '0.4s'}} />
