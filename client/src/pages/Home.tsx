  ...
  return (
    <section className="flex flex-col items-center text-white text-sm pb-20 px-4 font-poppins">
      ...
      <form
        ...
        className="bg-white/10 max-w-2xl w-full rounded-xl p-4 mt-10 border border-indigo-600/70"
      >
        <textarea
        ...
        <button
          type="submit"
          className="ml-auto mt-4 flex items-center gap-2 bg-gradient-to-r from-[#CB52D4] to-indigo-600 rounded-md px-4 py-2"
        >
          {!loading ? (
            'Create with AI'
          ) : (
            <>
              Creating...
              <Loader2Icon className="animate-spin w-4 h-4 text-white" />
            </>
          )}
        </button>
      </form>
