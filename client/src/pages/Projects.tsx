...
const togglePublish = async () => {}
  ...
  {/* builder navbar */}
    ...
    <div className='hidden sm:flex gap-2 bg-gray-950 p-1.5 rounded-md'>
      <Smartphone onClick={()=> setDevice('phone')} className={`size-6 p-1 rounded cursor-pointer ${device === 'phone' ? "bg-gray-700" : ""}`} />
  ...
  {project.isPublished ?  
    ...
    <div className='flex-1 flex overflow-auto'>
      ...
      <div className='flex-1 p-2 pl-0'><ProjectPreview ref={previewRef} project={project} isGenerating = {isGenerating} device={device}/></div>
    </div>
    </div>
  )
  :
